...
float sand_speckle_height(vec2 point) {
    // Use a high frequency for fine details suitable for normals
    float speckle = fract(sin(dot(point * 100.0, vec2(12.9898, 78.233))) * 43758.5453);
    // Use smoothstep for less harsh transitions than step()
    speckle = step(0.9, speckle);
    // Return a small height value based on the speckle intensity
    return speckle * 0.02; // Adjust multiplier for desired bump height
}

vec3 tex_sand_normal(vec2 point) {
    float epsilon = 1.0 / 512.;
    vec2 eps_x = vec2(epsilon, 0.0);
    vec2 eps_y = vec2(0.0, epsilon);

    float h_x_plus = sand_speckle_height(point + eps_x);
    float h_x_minus = sand_speckle_height(point - eps_x);
    float h_y_plus = sand_speckle_height(point + eps_y);
    float h_y_minus = sand_speckle_height(point - eps_y);

    float dHdx = (h_x_plus - h_x_minus) / (2.0 * epsilon);
    float dHdy = (h_y_plus - h_y_minus) / (2.0 * epsilon);

    float strength = 8.0; 
    vec3 normal = normalize(vec3(-dHdx * strength, -dHdy * strength, 1.0));

    vec3 normal_map_color = (normal + 1.0) * 0.5;

    return clamp(normal_map_color, 0.0, 1.0);
}
...
// =====================================================================
// ------------------------------------------------------------------
// Prozedurale Height-Map für Dünen + Körner (bleibt unverändert)
// ------------------------------------------------------------------
float duneHeight(vec2 uv) {
    // Großskalige Dünen
    float dunes  = perlin_fbm(uv * 2.0);
    // Feine Körner-Noise
    float grains = perlin_fbm(uv * 100.0) * 0.1;
    return dunes + grains;
}

const float EPS = 1.0 / 512.0; 

vec3 tex_sand_dune_normal(vec2 uv) {
    float hC  = duneHeight(uv);
    float hX1 = duneHeight(uv + vec2(EPS, 0.0));
    float hX2 = duneHeight(uv - vec2(EPS, 0.0));
    float hY1 = duneHeight(uv + vec2(0.0, EPS));
    float hY2 = duneHeight(uv - vec2(0.0, EPS));

    float dHdx = (hX1 - hX2) / (2.0 * EPS);
    float dHdy = (hY1 - hY2) / (2.0 * EPS);

    vec3 T = vec3(2.0 * EPS, 0.0, hX1 - hX2);
    vec3 B = vec3(0.0, 2.0 * EPS, hY1 - hY2);

    vec3 N = normalize(cross(T, B));

    float strength = 8.0;
    N = normalize(vec3(N.x * strength, N.y * strength, N.z));
